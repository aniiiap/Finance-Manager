...
export default function Layout() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-white to-purple-50 flex">
      ...
      <main className={`flex-1 ${!isSuperAdmin ? 'sm:pl-64' : ''} flex flex-col w-full`}>
        <header className="flex h-14 items-center gap-4 border-b border-slate-200/60 bg-white/70 backdrop-blur-md px-4 sm:px-6 sticky top-0 z-10">
          ...
          <div className={`flex-1 flex items-center font-semibold text-slate-900 ${!isSuperAdmin ? 'sm:hidden' : ''}`}>
          ...
          <div className="flex items-center gap-4">
            {user?.role === 'ADMIN' && (
             ...
             <div className="flex items-center gap-3">
               ...
               {isSuperAdmin && (
                 <button onClick={handleLogout} className="text-slate-500 hover:text-red-600 p-1 ml-2" title="Log out">
