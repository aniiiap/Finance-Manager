...
import { Folder, FileText, Image as ImageIcon, Upload, ArrowLeft, Trash2, ExternalLink, Loader2, X, Download } from 'lucide-react';
...
export default function ProgressReport() {
  ...
  const displayedFolders = projects || [];
        ...
        <Card className="shadow-sm border-t-4 border-t-slate-800">
          <CardHeader className="flex flex-row items-center justify-between border-b bg-slate-50 flex-wrap gap-4">
            <div className="flex items-center">
              ...
              <Folder className={`w-5 h-5 mr-2 ${activeTab === 'PDF' ? 'text-red-500' : 'text-blue-500'}`} />
          ...
          <CardContent className="p-6">
            {loading ? (
              ...
              <div className="text-center p-16 border-2 border-dashed rounded-lg text-muted-foreground">
                {activeTab === 'PDF' ? <FileText className="w-12 h-12 mx-auto mb-3 opacity-20" /> : <ImageIcon className="w-12 h-12 mx-auto mb-3 opacity-20" />}
                  ...
                  <TableBody>
                    {displayedFiles.map(file => (
                      <TableRow key={file.id}>
                        <TableCell>
                          {activeTab === 'PDF' ? <FileText className="w-5 h-5 text-red-500" /> : <ImageIcon className="w-5 h-5 text-blue-500" />}
                        </TableCell>
