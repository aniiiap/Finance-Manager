...
export default function Stock() {
  ...
  const filteredTransactions = inventoryTransactions.filter(tx => {
    ...
    let dateMatch = true;
    if (fromDate) dateMatch = dateMatch && new Date(tx.date) >= new Date(fromDate);
    if (toDate) dateMatch = dateMatch && new Date(tx.date) <= new Date(toDate + 'T23:59:59');
                    ...
                    <TableRow key={item.id}>
                      ...
                      {user?.role === 'ADMIN' && (
                        <TableCell className="flex gap-2">
                          <button onClick={() => { setItemToEdit(item); setIsEditItemModalOpen(true); }} className="text-indigo-400 hover:text-indigo-600 hover:scale-110 transition-all">
                            <Edit2 className="w-4 h-4" />
                          </button>
                          <button onClick={() => setItemToDelete(item.id)} className="text-rose-400 hover:text-rose-600 hover:scale-110 transition-all">
                ...
                <TableBody>
                  {paginatedTransactions.length === 0 ? (
                    ...
                    <TableRow key={tx.id} className={selectedTxIds.includes(tx.id) ? 'bg-red-50/50' : ''}>
                      ...
                      {user?.role === 'ADMIN' && (
                        <TableCell className="flex gap-2">
                          <button onClick={() => { setTxToEdit(tx); setTxType(tx.type); setTxItemId(tx.item_id); setIsEditTxModalOpen(true); }} className="text-indigo-400 hover:text-indigo-600 hover:scale-110 transition-all">
                            <Edit2 className="w-4 h-4" />
                          </button>
                          <button onClick={() => setTxToDelete(tx.id)} className="text-rose-400 hover:text-rose-600 hover:scale-110 transition-all">
                            <Trash2 className="w-4 h-4" />
                          </button>
                        </TableCell>
                      )}
